...
def get_company_names_that_pass_criteria (dict_investment_criteria, array_company_names_with_sufficient_data, \
  ...
  array_debt_to_equity_ratio = np.less( array_debt_to_equity_ratio, dict_investment_criteria['debt_to_equity_ratio'] )
  ...
  array_price_to_earnings_ratio = np.less( array_price_to_earnings_ratio, dict_investment_criteria['price_to_earnings_ratio'] )
  
  # Check if market capitalization is greater than $1 billion
  array_market_cap = np.greater( array_market_cap, dict_investment_criteria['market_cap'] )
  ...
  indices_stocks_pass_criteria = np.where( (array_debt_to_equity_ratio == True) & \
  ...
  company_names_that_pass_criteria = np.take(array_company_names_with_sufficient_data, indices_stocks_pass_criteria)
