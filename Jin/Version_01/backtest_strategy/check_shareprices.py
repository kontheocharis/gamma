...
class check_shareprices(object):
      ...
      for i in range(len(self.array_company_index_of_matrix)):
        ...
        for j in range(days_inbetween):
            ...
            if self.matrix[temp_buy_date_index + j][temp_company_index] > (2 * temp_buy_share_price):   # If there is no data for a date, it'll be NaN
              ...
              self.array_company_index_with_shareprices_gain.append( array_company_index_of_matrix[i] )
              self.array_date_index_with_shareprices_gain.append( temp_buy_date_index + j )
        ...
        if does_shareprice_increase == False:
          ...
          if sell_shareprice > temp_buy_share_price:
            ...
            self.array_company_names_with_shareprices_gain.append( array_company_names_that_pass_criteria[i] )  # Using i as index because the index within array (of stocks that pass criteria) is different from the company index. You will get an error if you write [temp_company_index] instead of [i]             
            self.array_company_index_with_shareprices_gain.append( array_company_index_of_matrix[i] )
            self.array_date_index_with_shareprices_gain.append( date_index_at_sell_date )
        ...
        if does_shareprice_increase == False:
          ...
          self.array_company_names_without_shareprice_gain.append( array_company_names_that_pass_criteria[i] )
