...
def get_companyindex(dict_companytickers, stock_ticker):
    '''To get company index of each stock ticker to add into a dictionary.'''
...
def convert_string_to_date(date_string):
    '''To convert string to date (in datetime format).'''
    ...
    date_string = date(int(date_string[0]), int(date_string[1]), int(
        date_string[2]))     # There are 3 parts since date(year, month, day)
    return date_string
